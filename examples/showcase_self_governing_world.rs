...
fn act1_rule_index() -> HashMap<CellType, Vec<Rule>> {
    ...
    idx.insert(CellType(PULSE), vec![Rule {
        ...
        changes: vec![], active_only: false, priority: 10, min_age: 0, overflow: Default::default(),
    }]);
    for k in 0..MAX_COUNT {
        ...
        rules.push(Rule {
            ...
            active_only: false, priority: 20, min_age: 0, overflow: Default::default(),
        });
        if k > 0 {
            rules.push(Rule {
                id: vec![counter_id], pattern: vec![(0, 0, counter_id)], shifts: vec![],
                changes: vec![(0, 0, ChangeValue::Literal(k))],
                active_only: false, priority: 10, min_age: QUIET_THRESHOLD, overflow: Default::default(),
            });
        }
...
fn act2_rule_index() -> HashMap<CellType, Vec<Rule>> {
    ...
    for acc in 0..ACC_MOD {
        ...
        rules.push(Rule {
            ...
            active_only: false, priority: 10, min_age: QUIET2, overflow: Default::default(),
        });
