...
fn snapshot(engine: &Engine<VecStorage>) -> Vec<Cell> {
    (0..WIDTH)
        .map(|x| engine.grid().get_cell(x, 0).copied().unwrap_or_default())
        .collect()
}
...
fn vote(snapshots: &[Vec<Cell>; 3]) -> (Vec<Cell>, Vec<usize>) {
    ...
    for (x, ((&a, &b), &c)) in snapshots[0]
        .iter()
        .zip(snapshots[1].iter())
        .zip(snapshots[2].iter())
        .enumerate()
    {
        let winner = if a == b {
            if c != a && !faulty.contains(&2) {
                faulty.push(2);
            }
            a
        } else if a == c {
            if !faulty.contains(&1) {
                faulty.push(1);
            }
            a
        } else if b == c {
...
fn main() {
    ...
    let mut replicas: Vec<Engine<VecStorage>> = (0..3)
        .map(|_| Engine::new(build_initial_grid(), rules.clone()))
        .collect();
    ...
    for tick in 1..=TICKS {
        ...
        if tick == FAULT_TICK {
            ...
            replicas[FAULT_REPLICA].grid_mut().set_cell(
                3,
                0,
                Cell {
                    value: CellValue::new(99),
                    born_at: 0,
                },
            );
        ...
        if faulty.is_empty() {
            ...
            assert_eq!(
                faulty,
                vec![FAULT_REPLICA],
                "голосование должно было указать именно на искусственно испорченную реплику"
            );
            repaired = true;
        }
    }
    ...
    assert!(
        repaired,
        "сценарий должен был хотя бы раз обнаружить и исправить сбой — иначе тест ничего не проверяет"
    );
    ...
    assert_eq!(
        final_snaps[0], final_snaps[1],
        "после исцеления реплики должны совпадать побитово"
    );
    assert_eq!(
        final_snaps[1], final_snaps[2],
        "после исцеления реплики должны совпадать побитово"
    );
