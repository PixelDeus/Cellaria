...
fn main() {
    ...
    for tick in 1..=2 {
        engine.run_tick();
        println!("тик {}: клетка = {:?} (правила: только 1->2)", tick, engine.grid().get_cell(0, 0).map(|c| c.value.0 .0));
    }
    ...
    for tick in 3..=4 {
        engine.run_tick();
        println!("тик {}: клетка = {:?} (правила: 1->2 и 2->3)", tick, engine.grid().get_cell(0, 0).map(|c| c.value.0 .0));
    }
