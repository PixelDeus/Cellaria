...
fn test_gpu_engine_run_ticks_matches_repeated_run_tick() {
    ...
    let mut idx: HashMap<CellType, Vec<Rule>> = HashMap::new();
    for v in 1u8..=5 {
        idx.entry(CellType(v))
            .or_default()
            .push(rule(v, vec![(0, 0, v)], v + 1, 0));
    }
    let initial = vec![(
        0usize,
        0usize,
...
fn test_gpu_engine_arbitrated_write_conflict_all_or_nothing() {
    ...
    assert_eq!(
        grid[1].value.0 .0, 1,
        "A must win the contested target (higher priority)"
    );
    assert_eq!(
        grid[2].value.0 .0, 2,
        "B's source must stay UNCHANGED (B rejected entirely, all-or-nothing)"
    );
    assert_eq!(grid[0].born_at, 1);
    assert_eq!(grid[1].born_at, 1);
