...
pub(crate) fn arbitrate_with_cam(
    ...
    // lookup доминирует стоимость цикла).
    //
    ...
    // реальный баг, найден на масштабе ~1M клеток.
...
pub(crate) fn arbitrate_spatial_with_cam(
    ...
    // РЕАЛЬНЫЙ БАГ, найден 2026-08-13 адверсариальным тестом (правило с
    // СИММЕТРИЧНЫМ affected-регионом — changes на смещениях -reach И
    // +reach разом, не однонаправленный сдвиг, как во всех более ранних
    // тестах этой области) — `safe_margin = margin + reach` (=3K, старое
    // значение) НЕДОСТАТОЧЕН. Прежнее обоснование ("худший boundary,
    // dist=margin-1, пишет максимум до margin-1+reach = safe_margin-1")
    // доказывает только, что boundary НЕ достигает порога safe_margin как
    // ЛИНИИ — оно не доказывает, что boundary не достигает КОНКРЕТНЫХ
    // клеток safe-матча. safe-матч на самой границе safe_margin имеет
    // клетки вплоть до `safe_margin - reach` (тянется НАЗАД, к краю, на
    // reach) — при safe_margin=3K это `2K`, а худший boundary дотягивается
    // до `3K-1` — диапазоны `[2K, 3K-1]` ПЕРЕСЕКАЮТСЯ для любого reach>=1,
    // если у правила есть клетки по ОБЕ стороны от центра (обычный
    // однонаправленный сдвиг физически не может реализовать этот случай,
    // поэтому не ловился раньше). Верный порог: `margin + 2*reach` (=4K)
    // — худший boundary (до `3K-1`) и худший safe (клетки от `4K-reach=3K`)
    // теперь РАЗДЕЛЕНЫ, не касаются. Используется ПОСЛЕ арбитража каждой
    // полосы (не для самого разбиения на core/boundary!) — все core-матчи
    // полосы (и "глубокие", и "рискованные") ОБЯЗАНЫ арбитрироваться
    // ВМЕСТЕ, в одном вызове `arbitrate_with_cam` per-полоса, иначе
    // соседние по позиции (но оба core) матчи одной и той же полосы
    // перестанут сравниваться друг с другом — именно так была сломана
    // первая версия этого фикса (разделяла core на safe/at_risk ДО
    // арбитража, x=103/x=104 в одной полосе перестали друг друга видеть).
    let safe_margin = margin + 2 * reach as u32;
    ...
    // ТОЙ ЖЕ полосы (несмотря на название "safe") -- at_risk-матч у самой
    // границы at_risk-зоны (dist приближается к safe_margin) с reach=K
    // может писать вплоть до почти safe_margin+K, то есть внутрь первых
    // "safe" клеток. Значит если at_risk-матч (L) проиграл ИМЕННО safe-матчу (не другому
