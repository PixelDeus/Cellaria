...
fn test_chunk_storage_basic() {
    ...
    assert_eq!(storage.width(), u32::MAX as usize);
    assert_eq!(storage.height(), u32::MAX as usize);
    // Default cell
    assert_eq!(storage.get(0, 0).unwrap().value, CellValue(CellType(0)));
    // Set and get
    storage.set(
    ...
    assert!(active.contains(&(10, 20)), "Active cells should include (10, 20)");
}
...
fn test_chunk_storage_get_mut_existing_cell() {
    ...
    assert_eq!(storage.get(100, 200).unwrap().value, CellValue(CellType(42)));
    let active: Vec<_> = storage.active_cells().collect();
    assert!(active.contains(&(100, 200)), "set should produce active cell");
